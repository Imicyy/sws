          ...
          <table id="pwdTable" class="table table-striped">
            ...
            <tbody>
              <?php foreach (($pwds ?? []) as $p): ?>
                ...
                <tr data-purok="<?= htmlspecialchars((string)($p['purok'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                  ...
                  <td><span class="badge badge-<?= ((($p['status'] ?? '') === 'Archived') ? 'secondary' : 'success') ?>"><?= htmlspecialchars((string)($p['status'] ?? ''), ENT_QUOTES, 'UTF-8') ?></span></td>
                  <td>
                    <a href="/pwd/<?= (int)($p['id'] ?? 0) ?>/application-pdf" class="btn btn-sm btn-print-app" target="_blank">PDF</a>
                  </td>
                </tr>
              <?php endforeach; ?>
            </tbody>

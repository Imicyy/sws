  ...
  <div class="container">
    ...
    async function generateReport() {
      try {
        ...
        let rows = [];
        if (Array.isArray(json.data) && json.data.length && (json.data[0].purok || json.data[0].name)) {
          rows = json.data.map(p => ({ purok: p.purok || p.name, count: p.count || p.total || 0 }));
        } else if (Array.isArray(json.data)) {
          const agg = {};
          json.data.forEach(s => { const k = (s.purok||s.address?.purok||'Unknown'); agg[k] = (agg[k]||0)+1; });
